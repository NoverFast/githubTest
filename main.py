...
if __name__ == '__main__':
    ...
    plots[1].plot(signalFreq, noisySignalAmp)
    plots[1].legend(["Noisy Signal"])
    plots[2].plot(sig.wiener(noisySignalAmp))
    plots[2].legend(["Wiener filter"])
    plots[3].plot(sig.medfilt(noisySignalAmp))
    plots[3].legend(["Median filter"])
    plots[4].plot(sig.savgol_filter(noisySignalAmp, 5, 2))
